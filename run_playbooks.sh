...
check_dependencies() { #
    ...
    then #
        ...
        # exit 1 # Allow script to continue if inventory is created later, or handle in ansible command
    fi #
} #

# Function to parse the main configuration file
parse_config() { #
    python3 -c "
import yaml
import sys
...
# Function to get domain platforms
    ...
    with open(config_file, 'r') as file:
        ...
        if value_found is None: #
            ...
            if len(global_feature_key_parts) > 0: #
                 # ex: enable_ssl -> GLOBAL_ENABLE_SSL ; #
...
# Function to get domain specific configuration
get_domain_config() { #
    local domain="$1" #
...
run_playbook() { #
    ...
    then #
        ...
        # extra_vars_json is for the entire --extra-vars payload structure later
        # No need to set extra_vars_json here as it's built into extra_vars_payload
    fi #
...
# Function to run global playbooks
run_global_playbooks() { #
    ...
    then #
        # Global SMTP might need a dedicated playbook if not per-domain
    ...
    then #
        # Global backup config might need a dedicated playbook
    ...
    then #
        # Global secure file permissions might need review or dedicated playbook
...
# Function to run domain specific playbooks
run_domain_specific_playbooks() { #
    ...
    # The is_feature_enabled function with "skip" as default helps determine if the key exists at all
    ...
    then #
        ...
        # Potentially call 14-setup-docker.yml with domain_context if it's adapted for per-domain compose files
    fi #
    ...
    then #
         # WordPress site security scan is part of global audit
...
main() { #
    ...
    if [ $? -ne 0 ] || [[ "$config_json" == "{}" && "$(wc -l < $CONFIG_FILE)" -gt 1 ]]; then #
